...
export function ColorPickerFabV4({
  ...
  const outerLarge = arcSpanDeg > 180 ? 1 : 0;
  const innerLarge = arcSpanDeg + 2 * alphaDiffDeg > 180 ? 1 : 0;
  const ribbonPath =
    `M ${leftInnerPt.x.toFixed(2)} ${leftInnerPt.y.toFixed(2)}` +
    ` L ${leftOuterPt.x.toFixed(2)} ${leftOuterPt.y.toFixed(2)}` +
    ` A ${ribbonOuter} ${ribbonOuter} 0 ${outerLarge} 1 ${rightOuterPt.x.toFixed(2)} ${rightOuterPt.y.toFixed(2)}` +
    ` L ${rightInnerPt.x.toFixed(2)} ${rightInnerPt.y.toFixed(2)}` +
    ` A ${effectiveInner} ${effectiveInner} 0 ${innerLarge} 0 ${leftInnerPt.x.toFixed(2)} ${leftInnerPt.y.toFixed(2)} Z`;
  ...
  const cx = fabCenter.x;
  const cy = fabCenter.y;
  ...
  return (
    <>
        ...
        {effOpen && (
          <>
            <motion.div
              ...
              style={{
                background: `radial-gradient(circle ${ribbonOuter + 120}px at ${cx}px ${cy}px, rgba(0,0,0,0.06) 0%, rgba(0,0,0,0.18) 55%, rgba(0,0,0,0.34) 100%)`,
              }}
              initial={{ opacity: 0, backdropFilter: "blur(0px)" }}
              animate={{ opacity: 1, backdropFilter: "blur(10px)" }}
              exit={{ opacity: 0, backdropFilter: "blur(0px)" }}
              transition={{ duration: openSec, ease: SOFT_EASE }}
            />
      ...
      <motion.button
        ...
        animate={{ scale: previewColor ? 1.06 : 1 }}
