...
<LocalVideo
  ...
  caption="Quite disappointing, really. Recursive generations almost immediately degrade to static, only slightly recovering before falling to pieces. This was the worst performing model in the test. Interesting only in how much it has changed since the original post."
/>
...
<LocalVideo
  ...
  caption="The only model that didn't degrade into noise, creating a coherent image on each generation. Mini clearly isn't just 'smaller' and there are more substantial differences in the models. Interestingly, across several tests, it consistently ended up with a variation of a demonic white guy... Coincidence?"
/>
...
Currently touted as the SOTA of image editing models, how does it degrade?

<LocalVideo
  src="/videos/nano_banana_pro_evolution.mp4?v=2"
  caption="Dwayne gets quite noisy by 10 recursions and devolves from there into fractals and eventually into something reminiscent of a broken LCD panel."
/>
...
<LocalVideo
  ...
  caption="The only model tested that got… hairy? Also notable for spontaneous suspenders and a large bearded ginger man with wings."
/>
...
<ModelChart
  ...
  revealIndex={3}
  order={["gpt", "gptMini", "nanoBananaPro", "seedream", "qwen", "nanoBanana", "flux"]}
  caption="The jump to the right at image 4 makes the SSIM trend less useful in this case."
/>
<RockVotePrompt model="seedream" />

<h2 id="qwen-image-edit">Qwen Image Edit</h2>

A highly trainable model which has recently been gaining in popularity on the release of a few nice fine tunes.
...
<LocalVideo
  ...
  caption="The model with the most significant Dwaynian motion, where the subject slides off to the side while turning to ectoplasm."
/>
...
I never really set out to learn anything with this; this was only ever a fun experiment. It also isn't surprising that there is loss in recursive generations with image editing models; of course, there is some loss in asking a generative model to reproduce an image, and that loss will compound over time. It is interesting, though, the breadth of variation in how these different models degrade. 
